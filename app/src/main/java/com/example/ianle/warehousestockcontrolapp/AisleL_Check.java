...
public class AisleL_Check extends AppCompatActivity {

    Button exitButton, finishButton;
    static TableLayout stockCheckTableL;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aisle_l__check);

        exitButton = (Button)findViewById(R.id.exitButton);
        finishButton = (Button)findViewById(R.id.finishButton);

        stockCheckTableL = (TableLayout)findViewById(R.id.stockCheckTableL);
        stockCheckTableL.setStretchAllColumns(true);
        ...
        try{
            ...
            for(String level : MainActivity.levels){
                for(int i = 1; i < MainActivity.aisleL.getLevel(level).length; i++){
                    ...
                    check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked){
                            if(check.isChecked()){
                                ...
                                stockCheckTableL.removeView(row);
                            }
                        }
                    });
                    row.addView(aisle);
                    ...
                    row.addView(boxes);
                    row.addView(check);
                    stockCheckTableL.addView(row, new TableLayout.LayoutParams(
            ...
            finishButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(stockCheckTableL.getChildCount() != 0){
                        Toast.makeText(getApplicationContext(), "Stock check finished. Loading finds page.", Toast.LENGTH_LONG).show();
                        Intent finds = new Intent("com.example.ianle.warehousestockcontrolapp.EditStock_L");
                        startActivity(finds);
                    }else{
                        // Update activity log
                        Date date = new Date();
            ...
            Toast.makeText(AisleL_Check.this, "System error", Toast.LENGTH_LONG).show();
        }
    }
}
