...
public class AisleM_Check extends AppCompatActivity {

    Button exitButton, finishButton;
    static TableLayout stockCheckTableM;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aisle_m__check);

        exitButton = (Button)findViewById(R.id.exitButton);
        finishButton = (Button)findViewById(R.id.finishButton);

        stockCheckTableM = (TableLayout)findViewById(R.id.stockCheckTableM);
        stockCheckTableM.setStretchAllColumns(true);
        ...
        try{
            ...
            for(String level : MainActivity.levels){
                for(int i = 1; i < MainActivity.aisleM.getLevel(level).length; i++){
                    ...
                    check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked){
                            if(check.isChecked()){
                                ...
                                stockCheckTableM.removeView(row);
                            }
                        }
                    });
                    row.addView(aisle);
                    ...
                    row.addView(boxes);
                    row.addView(check);
                    stockCheckTableM.addView(row, new TableLayout.LayoutParams(
            ...
            finishButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(stockCheckTableM.getChildCount() != 0){
                        Toast.makeText(getApplicationContext(), "Stock check finished. Loading finds page.", Toast.LENGTH_LONG).show();
                        Intent finds = new Intent("com.example.ianle.warehousestockcontrolapp.EditStock_M");
                        startActivity(finds);
                    }else{
                        // Update activity log
                        Date date = new Date();
            ...
            Toast.makeText(AisleM_Check.this, "System error", Toast.LENGTH_LONG).show();
        }
    }
}
