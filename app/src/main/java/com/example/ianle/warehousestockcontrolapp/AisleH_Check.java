...
public class AisleH_Check extends AppCompatActivity {

    Button exitButton, finishButton;
    static TableLayout stockCheckTableH;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aisle_h__check);

        exitButton = (Button)findViewById(R.id.exitButton);
        finishButton = (Button)findViewById(R.id.finishButton);

        stockCheckTableH = (TableLayout)findViewById(R.id.stockCheckTableH);
        stockCheckTableH.setStretchAllColumns(true);


        try{
            // Iterate over all levels of Aisle H
            ...
            for(String level : MainActivity.levels){
                for(int i = 1; i < MainActivity.aisleH.getLevel(level).length; i++){
                    ...
                    check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked){
                            if(check.isChecked()){
                                ...
                                stockCheckTableH.removeView(row);
                            }
                        }
                    });
                    row.addView(aisle);
                    ...
                    row.addView(boxes);
                    row.addView(check);
                    stockCheckTableH.addView(row, new TableLayout.LayoutParams(
            ...
            finishButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(stockCheckTableH.getChildCount() != 0){
                        Toast.makeText(getApplicationContext(), "Stock check finished. Loading finds page.", Toast.LENGTH_LONG).show();
                        Intent finds = new Intent("com.example.ianle.warehousestockcontrolapp.EditStock_H");
                        startActivity(finds);
                    }else{
                        // Update activity log
                        Date date = new Date();
            ...
            Toast.makeText(AisleH_Check.this, "System error", Toast.LENGTH_LONG).show();
        }
    }
}
