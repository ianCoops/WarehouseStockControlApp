...
public class AisleG_Check extends AppCompatActivity {

    Button exitButton, finishButton;
    static TableLayout stockCheckTable;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aisle_g_check);

        exitButton = (Button)findViewById(R.id.exitButton);
        finishButton = (Button)findViewById(R.id.finishButton);

        stockCheckTable = (TableLayout)findViewById(R.id.stockCheckTable);
        stockCheckTable.setStretchAllColumns(true);
        ...
        try{
            ...
            for(String level : MainActivity.levels){
                for(int i = 1; i < MainActivity.aisleG.getLevel(level).length; i++){
                    ...
                    check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked){
                            if(check.isChecked()){
                                ...
                                stockCheckTable.removeView(row);
                            }
                        }
                    });
                    row.addView(aisle);
                    ...
                    row.addView(boxes);
                    row.addView(check);
                    stockCheckTable.addView(row, new TableLayout.LayoutParams(
            ...
            finishButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(stockCheckTable.getChildCount() != 0){
                        Toast.makeText(getApplicationContext(), "Stock check finished. Loading finds page.", Toast.LENGTH_LONG).show();
                        Intent finds = new Intent("com.example.ianle.warehousestockcontrolapp.EditStock_G");
                        startActivity(finds);
                    }else{
                        // Update activity log
                        Date date = new Date();
            ...
            Toast.makeText(AisleG_Check.this, "System error", Toast.LENGTH_LONG).show();
        }
    }
}
