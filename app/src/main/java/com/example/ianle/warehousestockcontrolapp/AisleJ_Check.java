...
public class AisleJ_Check extends AppCompatActivity {

    Button exitButton, finishButton;
    static TableLayout stockCheckTableJ;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aisle_j__check);

        exitButton = (Button)findViewById(R.id.exitButton);
        finishButton = (Button)findViewById(R.id.finishButton);

        stockCheckTableJ = (TableLayout)findViewById(R.id.stockCheckTableJ);
        stockCheckTableJ.setStretchAllColumns(true);
        ...
        try{
            ...
            for(String level : MainActivity.levels){
                for(int i = 1; i < MainActivity.aisleJ.getLevel(level).length; i++){
                    ...
                    check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener(){
                        ...
                        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked){
                            if(check.isChecked()){
                                ...
                                stockCheckTableJ.removeView(row);
                            }
                        }
                    });
                    row.addView(aisle);
                    ...
                    row.addView(boxes);
                    row.addView(check);
                    stockCheckTableJ.addView(row, new TableLayout.LayoutParams(
            ...
            finishButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(stockCheckTableJ.getChildCount() != 0){
                        Toast.makeText(getApplicationContext(), "Stock check finished. Loading finds page.", Toast.LENGTH_LONG).show();
                        Intent finds = new Intent("com.example.ianle.warehousestockcontrolapp.EditStock_J");
                        startActivity(finds);
                    }else{
                        // Update activity log
                        Date date = new Date();
            ...
            Toast.makeText(AisleJ_Check.this, "System error", Toast.LENGTH_LONG).show();
        }
    }
}
